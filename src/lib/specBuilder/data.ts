/* Spec Builder — materials database.
   Direct port of design/spec-builder-data.js (window.SB_MATERIALS).
   Kept flat (not modularized) so the scoring engine stays simple
   and inspectable. 23 materials. */

import type { Application, Environment, StrengthLevel } from "./options";

export type Companion = { spec: string; grade: string };
...
export type Material = {
  ...
  isAlloySteel?: boolean;
  isStainless: boolean;
  isNickelAlloy: boolean;
  nuts: Companion[];
  washers: Companion[];
  apps: Application[];
  envs: Environment[];
  strengths: StrengthLevel[];
  why: { default: string } & Partial<Record<Application, string>>;
};
